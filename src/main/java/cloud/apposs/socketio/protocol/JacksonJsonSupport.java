...
import cloud.apposs.socketio.ack.MultiTypeAckCallback;
...
public class JacksonJsonSupport implements JsonSupport {
    protected class AckArgsDeserializer extends StdDeserializer<AckArgs> {
        private static final long serialVersionUID = 2810461017389946707L;
    ...
    protected class EventDeserializer extends StdDeserializer<Event> {
        ...
        public Event deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
            ...
            if (!eventMapping.containsKey(ek)) {
                ek = new EventKey(SocketIOConstants.DEFAULT_NAMESPACE, eventName);
                if (!eventMapping.containsKey(ek)) {
            ...
            while (true) {
                ...
                if (i > eventClasses.size() - 1) {
                    Logger.debug("Event %s has more args than declared in handler: %s", eventName, null);
                    break;
                }
    ...
    public void addEventMapping(String namespaceName, String eventName, Class<?> ... eventClass) {
        EventKey eventKey = new EventKey(namespaceName, eventName);
        List<Class<?>> eventMapping = eventDeserializer.eventMapping.computeIfAbsent(
                eventKey, k -> new ArrayList<Class<?>>()
        );
        eventMapping.addAll(Arrays.asList(eventClass));
    }
